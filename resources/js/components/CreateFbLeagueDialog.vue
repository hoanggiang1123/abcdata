<template>
    ...
    >
        <v-card>
            ...
            <v-card-text>
                <v-form @submit.prevent="handleCreateFbLeague">
                    <v-row>
...
export default {
    ...
    methods: {
        ...mapActions('fbStat', ['createLeague']),
        handleCreateFbLeague () {
            if (this.name === '' || this.name.trim() === '' || this.slug === '' || this.slug.trim() === '') {
                alert('Name and Slug are requied');
                return false;
            }
