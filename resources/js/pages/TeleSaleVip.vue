<template>
     <v-row>
        ...
        <CreateTeleSaleVipDialog :openCreateDialog="openCreateDialog" @close-create-dialog="handleCloseCreateDialog" />
     </v-row>
</template>

<script>
import { mapGetters, mapActions } from 'vuex';
import CreateTeleSaleVipDialog from '../components/CreateTeleSaleVipDialog';
export default {
    components: {
        CreateTeleSaleVipDialog
    },
    data () {
        return {
            headers: [
    ...
    computed: {
        ...mapGetters('teleSale', {
            vips: 'getTeleSaleVip',
            pagination: 'getTeleSaleVipPagination'
        })
    },
    created () {
        this.loadPageData();
    ...
    methods: {
        ...mapActions('teleSale', ['getTeleSaleVip', 'updateTeleSaleVip', 'deleteTeleSaleVip', 'getTeleSaleLineOwn']),
        async loadPageData () {
            this.loading = true;
            try {
                await this.getTeleSaleVip({
                    name: this.name,
        ...
        handleGoToTeleSale (name) {
            this.$router.push('/data?vip_id=' + name);
        },
        handleDeleteLine (id) {
             const deleteConfirm = confirm('Are you sure want to delete, it might affect to tele sale data ?');
