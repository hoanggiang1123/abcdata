        ...
        <v-col cols="12">
            ...
            >
                ...
                <template #[`item.folder_id`]="{item}">
                    <v-list-item style="color: gray;">{{ item.folder && item.folder.name ? item.folder.name : '' }}</v-list-item>
                </template>
...
export default {
    ...
    data () {
        return {
            headers: [
                ...
                { text: 'Action', value: 'action', align: 'center',  width: '25%', sortable: false},
            ],
            selected: [],
            loading: false,
            name: '',
    ...
    async created () {
        try {
            await Promise.all([
                ...
                this.getEnterLinkFilter({ id: this.$route.params.id })
            ]);
        } catch (err) {
            console.log(err)
        }
    ...
    methods: {
        ...mapActions('enterLink', ['getEnterLinkList', 'getEnterLinkFilter', 'getFolderLinkList', 'updateEnterLink', 'createManyEnterLink', 'deleteManyLink', 'deleteLink']),
