<template>
     <v-row>
        ...
        <CreateTeleSaleCategoryDialog :openCreateDialog="openCreateDialog" @close-create-dialog="handleCloseCreateDialog" />
     </v-row>
</template>

<script>
import { mapGetters, mapActions } from 'vuex';
import CreateTeleSaleCategoryDialog from '../components/CreateTeleSaleCategoryDialog';
export default {
    components: {
        CreateTeleSaleCategoryDialog
    },
    data () {
        return {
            headers: [
    ...
    computed: {
        ...mapGetters('teleSale', {
            categories: 'getTeleSaleCategory',
            pagination: 'getTeleSaleCategoryPagination'
        })
    },
    created () {
        this.loadPageData()
    ...
    methods: {
        ...mapActions('teleSale', ['getTeleSaleCategory', 'updateTeleSaleCategory', 'deleteTeleSaleCategory']),
        async loadPageData () {
            this.loading = true;
            try {
                await this.getTeleSaleCategory({
                    name: this.name,
        ...
        handleDeleteMany () {
            ...
            if (deleteConfirm) {
                ...
                this.loading = true;
                this.deleteTeleSaleCategory({ ids })
                .then(resp => {
                    this.loadPageData()
                    this.loading = false;
                }).catch (err => {
        ...
        handleSaveItem (index) {
            ...
            this.loading = true;
            this.updateTeleSaleCategory(update)
                .then(resp => {
                    this.resetPageData();
                    this.loading = false;
                })
        ...
        handleGoToTeleSale (name) {
            this.$router.push('/data?category_id=' + name);
        },
        handleDeleteLine (id) {
             const deleteConfirm = confirm('Are you sure want to delete, it might affect to tele sale data ?');
            ...
            if (deleteConfirm) {
                this.loading = true;
                this.deleteTeleSaleCategory({ ids: [id] })
                .then(resp => {
                    this.loadPageData()
                    this.loading = false;
                }).catch (err => {
